...
const loadDynamicIcons = async () => { try { const response = await apiService.components.getAll(); setDynamicIcons(response.data.filter(comp => comp.isActive)); } catch (error) { console.error("Error cargando íconos dinámicos:", error); } finally { setLoading(false); } };
